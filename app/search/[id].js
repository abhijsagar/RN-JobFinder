...
const JobSearch = () => {
    ...
    const handleSearch = () => {
        ...
        try {
            let empType = '';
            if (params.id === 'Full-time') {
                empType = 'FULLTIME';
            } else if (params.id === 'Part-time') {
                empType = 'PARTTIME';
            } else if (params.id === 'Contractor') {
                empType = 'CONTRACTOR';
            }
            if (empType !== '') {
                const response = jobData.filter((item) => item.job_employment_type === empType);
                setSearchResult(response);
            } else {
                const response = jobData.filter((item) => item.job_title.includes(params.id));
                setSearchResult(response);
            }
        } catch (error) {
